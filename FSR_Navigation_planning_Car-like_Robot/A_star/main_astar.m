...
params.heuristic_type = 2;   % Heuristic type: 1 = Euclidean distance, 2 = Manhattan distance
...
figure('Name', 'A* Path Planning', 'NumberTitle', 'off', 'Position', [100 100 800 600]);
imshow(map);
hold on;
...
title('A* Path Planning', 'FontSize', 12);
...
title(['A* Path Planning - Heuristic: ' heuristic_name ' - Path nodes: ' num2str(path_node_count) ' | Total explored: ' num2str(tree_data.counter)], 'FontSize', 12);
...
if ~isempty(path_points)
    ...
    % Save the plot with proper legend
       ...
       % filename = [save_path, 'astar_path.pdf'];
    ...
    % Save trajectory data
     % save('trajectory_data_astar.mat', 'xd', 'yd', 'thetad', 'phid', 'q0');
    ...
    fprintf('Trajectory data saved to trajectory_data_astar.mat\n');
else
